...
from currency_exchange.currency_exchange.fapiadoption.appadapter import (
...
async def app(local_sessionmaker):
	...
	fapi_app.dependency_overrides = {
		jwt_revocation_checker_provider: jwt_token_revocation_checker_stub
	}
	return fapi_app
...
async def app_user(local_sessionmaker):
	...
	user = User(
		username="ce_client", password="abrakadabra", category=UserCategory.API_CLIENT
	)
...
async def admin_user(local_sessionmaker):
	session = local_sessionmaker()
	user = User(
		username="ce_admin", password="abrakadabra", category=UserCategory.ADMIN
	)
	async with session.begin():
		session.add(user)
	session.close()
...
class TestCurrenciesEndpoints:
	...
	async def test_get_all_currencies_successful(
		self, access_token, request_client, db_session
	):
		response = await request_client.get(
			self.all_currencies_endpoint,
	...
	async def test_get_currency_successful(
		self, access_token, request_client, db_session, get_currency_request_endpoint
	):
		response = await request_client.get(
			get_currency_request_endpoint("USD"),
			headers={"Authorization": f"Bearer {access_token[0]}"},
		)
	...
	async def test_get_currency_error_when_currency_doesnt_exist(
		self, access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.get(
			get_currency_request_endpoint("XXX"),
			headers={"Authorization": f"Bearer {access_token[0]}"},
		)
		assert response.status_code == 404

	async def test_get_currency_error_when_currency_code_not_present_in_url(
		self, access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.get(
	...
	async def test_get_currency_error_when_currency_code_is_invalid(
		self, code, access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.get(
			get_currency_request_endpoint(code),
			headers={"Authorization": f"Bearer {access_token[0]}"},
		)
		assert response.status_code == 400

	async def test_update_currency_successful(
		self, access_token, request_client, get_currency_request_endpoint
	):
	...
	async def test_update_currency_error_when_currency_doesnt_exist(
		self, access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.patch(
			get_currency_request_endpoint("XXX"),
	...
	@pytest.mark.parametrize(
		"data",
		[
			{"code": "X", "name": "X", "sign": "X"},
			{"code": "USD", "name": None, "sign": None},
			{"code": "", "name": "X", "sign": "X"},
			{"code": "USD", "nme": "X", "sgn": "X"},
		],
	)
	async def test_update_currency_error_when_bad_data_in_request(
		self, data, access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.patch(
			get_currency_request_endpoint(data["code"]),
			headers={"Authorization": f"Bearer {access_token[0]}"},
	...
	async def test_add_currency_successful(
		self, access_token, request_client, db_session
	):
		code, name, sign = "AMD", "Dram", "X"
		response = await request_client.post(
			self.add_currency_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
	...
	async def test_add_currency_error_when_currency_already_exists(
		self, access_token, request_client
	):
		response = await request_client.post(
			self.add_currency_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
	...
	@pytest.mark.parametrize(
		"data",
		[
			{"code": "AF", "name": "X", "sign": "X"},
			{"code": "USD", "name": None, "sign": "X"},
			{"code": "USD", "name": "X", "sign": None},
			{"code": "USD", "nae": "X", "sgn": "X"},
		],
	)
	async def test_add_currency_error_when_bad_data_in_request(
		self, data, access_token, request_client
	):
		response = await request_client.post(
			self.add_currency_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
	...
	async def test_delete_currency_successful(
		self,
		admin_access_token,
		request_client,
		db_session,
	...
	async def test_delete_currency_error_when_currency_doesnt_exist(
		self, admin_access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.delete(
			get_currency_request_endpoint("XXX"),
			headers={"Authorization": f"Bearer {admin_access_token[0]}"},
	...
	async def test_delete_currency_error_when_bad_data_in_request(
		self, code, admin_access_token, request_client, get_currency_request_endpoint
	):
		response = await request_client.delete(
			get_currency_request_endpoint(code),
			headers={"Authorization": f"Bearer {admin_access_token[0]}"},
...
class TestExchangeRatesEndpoints:
	...
	async def test_get_all_exchange_rates(
		...
		response = await request_client.get(
			self.get_all_exch_rates_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
		)
	...
	async def test_get_exchange_rate_successful(
		...
		response = await request_client.get(
			get_exchange_rate_request_endpoint("EURUSD"),
			headers={"Authorization": f"Bearer {access_token[0]}"},
		)
	...
	async def test_get_exchange_rate_error_when_exchange_rate_doesnt_exist(
		self, access_token, request_client, get_exchange_rate_request_endpoint
	):
		response = await request_client.get(
			get_exchange_rate_request_endpoint("EURRUB"),
			headers={"Authorization": f"Bearer {access_token[0]}"},
		)
	...
	async def test_get_exchange_rate_error_when_bad_data_in_request(
		self, codes, access_token, request_client, get_exchange_rate_request_endpoint
	):
		response = await request_client.get(
			get_exchange_rate_request_endpoint(codes),
			headers={"Authorization": f"Bearer {access_token[0]}"},
			follow_redirects=True,
		)
	...
	async def test_update_exchange_rate_successful(
		self,
		access_token,
		request_client,
		db_session,
		...
		new_rate = 80
		response = await request_client.patch(
			get_exchange_rate_request_endpoint("RUBUSD"),
			headers={"Authorization": f"Bearer {access_token[0]}"},
			data={"rate": new_rate},
		)
	...
	async def test_update_exchange_rate_error_when_rate_doesnt_exist(
		self, access_token, request_client, get_exchange_rate_request_endpoint
	):
		new_rate = 80
		response = await request_client.patch(
			get_exchange_rate_request_endpoint("XXXYYY"),
			headers={"Authorization": f"Bearer {access_token[0]}"},
			data={"rate": new_rate},
		)
	...
	async def test_update_exchange_rate_error_when_bad_data_in_request(
		self, data, access_token, request_client, get_exchange_rate_request_endpoint
	):
		response = await request_client.patch(
			get_exchange_rate_request_endpoint(data["codes"]),
			headers={"Authorization": f"Bearer {access_token[0]}"},
	...
	async def test_add_exchange_rate_successful(
		self, access_token, request_client, db_session
	):
		rate = 1.15
		response = await request_client.post(
			self.add_exch_rate_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
			data={"baseCurrencyCode": "DKK", "targetCurrencyCode": "USD", "rate": rate},
		)
	...
	async def test_add_exchange_rate_error_when_rate_already_exists(
		self, access_token, request_client
	):
		response = await request_client.post(
			self.add_exch_rate_endpoint,
	...
	async def test_add_exchange_rate_error_when_same_currency(
		self, access_token, request_client
	):
		response = await request_client.post(
			self.add_exch_rate_endpoint,
	...
	async def test_add_exchange_rate_error_when_bad_data_in_request(
		self, data, access_token, request_client
	):
		response = await request_client.post(
			self.add_exch_rate_endpoint,
	...
	async def test_add_exchange_rate_error_when_currency_doesnt_exist(
		self, access_token, request_client
	):
		response = await request_client.post(
			self.add_exch_rate_endpoint,
	...
	async def test_delete_exchange_rate_successful(
		self,
		admin_access_token,
		request_client,
		db_session,
	...
	async def test_delete_exchange_rate_error_when_rate_doesnt_exist(
		self, admin_access_token, request_client, get_exchange_rate_request_endpoint
	):
		response = await request_client.delete(
			get_exchange_rate_request_endpoint("XXXYYY"),
			headers={"Authorization": f"Bearer {admin_access_token[0]}"},
...
class TestConvertionEndpoint:
	...
	async def test_convert_currency_straight_strat_successful(
		self, access_token, request_client, db_session
	):
		amount = 1000
		response = await request_client.get(
			self.convertion_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
			params={"from_": "RUB", "to": "USD", "amount": amount},
		)
		assert response.status_code == 200

		er = await get_exchange_rate_from_db("RUB", "USD", db_session)
		response_data = response.json()
		assert response_data["convertedAmount"] == pytest.approx(
			er.value.value * amount
		)

	async def test_convert_currency_reversed_strat_successful(
		self, access_token, request_client, db_session
	):
		amount = 100
		response = await request_client.get(
			self.convertion_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
			params={"from_": "USD", "to": "RUB", "amount": amount},
		)
		assert response.status_code == 200
		...
		response_data = response.json()
		assert response_data["convertedAmount"] == pytest.approx(
			1 / er.value.value * amount
		)

	async def test_convert_currency_cross_rate_strat_successful(
		self, access_token, request_client, db_session
	):
		amount = 100
		response = await request_client.get(
			self.convertion_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
			params={"from_": "EUR", "to": "RUB", "amount": amount},
		)
		assert response.status_code == 200
		...
		response_data = response.json()
		assert response_data["convertedAmount"] == pytest.approx(
			er1.value.value / er2.value.value * amount
		)

	async def test_convert_currency_error_when_rate_doesnt_exist(
		self, access_token, request_client
	):
		response = await request_client.get(
			self.convertion_endpoint,
			headers={"Authorization": f"Bearer {access_token[0]}"},
			params={"from_": "GBP", "to": "KZT", "amount": 100},
		)
	...
	@pytest.mark.parametrize(
		"data",
		[
			{"from": "US", "to": "EUR", "amount": 1000},
			{"from": "USD", "to": "EUR", "amount": 0},
			{"from": "USD", "to": "EUR", "amount": -1},
			{"from": None, "to": "EUR", "amount": 1000},
			{"frm": "USD", "to": "EUR", "amount": 1000},
			{"from": None, "to": "EUR", "more": "yes", "amount": 1000},
		],
	)
	async def test_convert_currency_error_when_bad_data_in_request(
		self, data, access_token, request_client
	):
		response = await request_client.get(
			self.convertion_endpoint,
