...
from currency_exchange.currency_exchange.application.dto import GetExchangeRateDto, MakeConvertionDto
from currency_exchange.currency_exchange.application.interactions.exchangeratesinteracions import (
    GetExchangeRateInteraction, ConvertCurrencyInteraction
)
...
async def test_get_exchange_rate_interaction_by_reversed_rate_successful(get_exchange_rate_interaction,
    ...
    assert res.rate.value == pytest.approx(exchange_rates_models[('USD', 'EUR')].value.value)



async def test_get_exchange_rate_interaction_by_common_rate_successful(get_exchange_rate_interaction,
                                                                       exchange_rates_models):
    res = await get_exchange_rate_interaction(
...
async def test_get_exchange_rate_interaction_error_when_by_straight_rate(get_exchange_rate_interaction):
    with pytest.raises(errors.ExchangeRateDoesntExistError):
        await get_exchange_rate_interaction(GetExchangeRateDto(CurrencyCode('RUB'), CurrencyCode('EUR')),
                                            rate_fetch_strategy=ERFetchStrat.BY_STRAIGHT_RATE)


async def test_get_exchange_rate_interaction_error_when_by_reversed_rate(get_exchange_rate_interaction):
    with pytest.raises(errors.ExchangeRateDoesntExistError):
        await get_exchange_rate_interaction(GetExchangeRateDto(CurrencyCode('RUB'), CurrencyCode('EUR')),
                                            rate_fetch_strategy=ERFetchStrat.BY_REVERSED_RATE)


async def test_get_exchange_rate_interaction_error_when_by_common_rate(get_exchange_rate_interaction):
    with pytest.raises(errors.ExchangeRateDoesntExistError):
        await get_exchange_rate_interaction(GetExchangeRateDto(CurrencyCode('KZT'), CurrencyCode('GBP')),
                                            rate_fetch_strategy=ERFetchStrat.BY_COMMON_CURRENCY)


async def test_convert_currency_interaction_successful_when_cross_rate_is_needed(convert_currency_interaction,
                                                                                 exchange_rates_models):
    rate_val = exchange_rates_models['RUB', 'USD'].value.value / exchange_rates_models[('EUR', 'USD')].value.value
    amount = 10
