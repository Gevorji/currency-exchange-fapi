...
from currency_exchange.currency_exchange.domain.entities import (
	Currency,
...
def test_exchange_rate_incorrect_rate_value_error(rate):
	with pytest.raises(errors.IncorrectExchangeRateValue):
		CurrenciesExchangeRate(
			...
			Currency(
				CurrencyCode("RUB"), CurrencySign("Р"), CurrencyName("Russian ruble")
			),
...
@pytest.mark.parametrize(
	...
	[
		[
			CurrenciesExchangeRate(
				Currency(
					CurrencyCode("USD"), CurrencySign("$"), CurrencyName("US dollar")
				),
				Currency(
					CurrencyCode("RUB"),
					CurrencySign("Р"),
