...
async def get_currency_from_db(code: str, session: AsyncSession) -> CurrencyORMModel:
    res = await session.scalars(select(CurrencyORMModel).where(CurrencyORMModel.code == code))
    return res.one_or_none()


async def get_exchange_rate_from_db(basec: str, targetc: str, session: AsyncSession) -> CurrenciesExchangeRateORMModel:
