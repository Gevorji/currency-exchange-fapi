...
@test_router.get("/guarded_endpoint", dependencies=[Security(verify_access, scopes=REQUIRED_ACCESS_SCOPES)])
...
@pytest.fixture
async def app(token_validator_dependency_override, revocation_checker_dependency_override):
    _app = FastAPI()
...
@pytest.mark.parametrize(
    'scopes', [['elf', 'human', 'middle_earth', 'fish_recipie_defender'], ['elf', 'human', 'middle_earth'], ['all']]
)
async def test_verify_access_success(token_issuer, request_client: AsyncClient, scopes):
    ...
    response = await request_client.get('/guarded_endpoint', headers={'Authorization': f'Bearer {access_token}'})
    assert response.status_code == 200


@pytest.mark.parametrize('scopes', [['elf', 'human'], ['elf']])
async def test_verify_access_error_when_token_does_not_have_required_scopes(token_issuer, request_client: AsyncClient, scopes):
    access_token = token_issuer.get_access_token(subject='Smeagol', scope=scopes)[0]
...
async def test_verify_access_error_when_token_is_refresh(token_issuer, request_client: AsyncClient):
    token = token_issuer.get_refresh_token(subject='Smeagol', scope=['elf', 'human', 'middle_earth'])[0]

    response = await request_client.get('/guarded_endpoint', headers={'Authorization': f'Bearer {token}'})
    with pytest.raises(httpx.HTTPStatusError):
        response.raise_for_status()
...
async def test_verify_access_error_when_token_revoked(
    ...
    access_token, _, payload = token_issuer.get_access_token(subject='Smeagol', scope=REQUIRED_ACCESS_SCOPES)
    del app.dependency_overrides[jwt_revocation_checker_provider]

    await token_state_repo.save(
        TokenStateDbIn(
            id=payload['jti'], type='access', device_id='none', expiry_date=payload['exp'],
            revoked=True, user_id=users_models['Bilbo_baggins'].id
        )
    )
...
async def test_verify_access_error_when_token_has_no_token_state_in_db(token_issuer, request_client: AsyncClient, app):
    access_token, _, payload = token_issuer.get_access_token(subject='Smeagol', scope=REQUIRED_ACCESS_SCOPES)
    del app.dependency_overrides[jwt_revocation_checker_provider]
