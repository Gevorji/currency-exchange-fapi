...
from .utils import EncodedToken, get_user_from_db, add_token_state_to_db, get_token_state_from_db
...
@pytest.fixture
async def admin_access_token(admin_token_issuer, admin_user, db_session, mock_token_issuers_encryption_keys):
    token = EncodedToken(*admin_token_issuer.get_access_token(scope=['all']))
    await add_token_state_to_db(
        TokenState(
        id=token.payload['jti'], user_id=admin_user.id, type='access',
        device_id='none', expiry_date=datetime.datetime.fromtimestamp(token.payload['exp'])
        ), db_session
    )
    return token
...
async def usual_access_token(admin_token_issuer, users_models, db_session):
    ...
    await add_token_state_to_db(
        TokenState(
            id=token.payload['jti'], user_id=user.id, type='access',
            device_id='none', expiry_date=datetime.datetime.fromtimestamp(token.payload['exp'])
        ), db_session
    )
    return token
...
@pytest.mark.parametrize('request_url,method,expected_status_code', [('/admin/users/all', 'get', 200)])
async def test_admin_resource_access_success(request_url, request_client, method, admin_access_token, expected_status_code):
    method_func = getattr(request_client, method)
    response = await method_func(request_url, headers={'Authorization': f'Bearer {admin_access_token.str}'})
...
async def test_admin_resource_access_error(request_url, request_client, method, usual_access_token):
    method_func = getattr(request_client, method)
    response = await method_func(request_url, headers={'Authorization': f'Bearer {usual_access_token.str}'})

    assert response.status_code == 403


async def test_admin_get_all_users_success(admin_access_token, request_client, db_session):
    response = await request_client.get('/admin/users/all', headers={'Authorization': f'Bearer {admin_access_token.str}'})

    assert response.status_code == 200

    response_data = response.json()

    number_of_users_in_db = await db_session.execute(select(func.count()).select_from(User))
...
async def test_get_one_user_success(admin_access_token, request_client, users_models):
    ...
    response = await request_client.get(
        '/admin/users/search', headers={'Authorization': f'Bearer {admin_access_token.str}'},
...
async def test_get_one_user_user_not_found_error(admin_access_token, request_client, users_models):
    user = users_models['Bilbo_baggins']
    response = await request_client.get(
        '/admin/users/search', headers={'Authorization': f'Bearer {admin_access_token.str}'},
...
class TestPromoteUsersCategory:
    ...
    async def test_promote_users_category_success(
            self, admin_access_token, request_client, users_models, get_request_url, db_session
    ):
        user = users_models['Bilbo_baggins']
        response = await request_client.patch(
            get_request_url(user.id), headers={'Authorization': f'Bearer {admin_access_token.str}'},
            params={'to': '0'}
        )
        assert response.status_code == 200
    ...
    async def test_promote_users_category_user_not_found_error(
        ...
        user = users_models['Bilbo_baggins']
        response = await request_client.patch(
            get_request_url(user.id*1000), headers={'Authorization': f'Bearer {admin_access_token.str}'},
            params={'to': '0'}
        )
        assert response.status_code == 404
...
class TestDeactivateUser:
    ...
    async def test_deactivate_user_success(
            self, user, request_client, admin_access_token, existing_tokens, db_session, get_request_url
    ):
        response = await request_client.patch(
            get_request_url(user.id), headers={'Authorization': f'Bearer {admin_access_token.str}'}
        )
        ...
        token_states = [await get_token_state_from_db(token.payload['jti'], db_session) for token in tokens]
    ...
    async def test_deactivate_user_user_is_admin_error(
        ...
        response = await request_client.patch(
            get_request_url(admin_user.id), headers={'Authorization': f'Bearer {admin_access_token.str}'}
        )
...
class TestActivateUser:
    ...
    async def test_activate_user_success(
            self, users_models, request_client, admin_access_token, db_session, get_request_url
    ):
        user = users_models['The_Goblin_King']
        response = await request_client.patch(
            get_request_url(user.id), headers={'Authorization': f'Bearer {admin_access_token.str}'}
