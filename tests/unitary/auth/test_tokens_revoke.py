...
async def test_revoke_all_users_tokens_success(
	...
	tokens_states = [
		await get_token_state_from_db(t.payload["jti"], db_session)
		for t in existing_tokens["none"].values()
	] + [
		await get_token_state_from_db(t.payload["jti"], db_session)
		for t in existing_tokens["tel"].values()
	]
...
async def test_revoke_specified_users_tokens_success(
	...
	revoked_tokens_ids = [
		t.payload["jti"]
		for t in [existing_tokens["tel"]["access"], existing_tokens["none"]["access"]]
	]
	untouched_tokens_ids = [
		t.payload["jti"]
		for t in [existing_tokens["tel"]["refresh"], existing_tokens["none"]["refresh"]]
	]
	response = await request_client.patch(
		request_url,
		headers={
	...
	revoked_tokens_states = [
		await get_token_state_from_db(t_id, db_session) for t_id in revoked_tokens_ids
	]
	untouched_tokens_states = [
		await get_token_state_from_db(t_id, db_session) for t_id in untouched_tokens_ids
	]
...
async def test_revoke_tokens_when_user_have_no_active_tokens_success(
	...
	request_client,
	request_url,
	existing_revoked_refresh_token,
	db_session,
	users_raw_passwords,
):
	response = await request_client.patch(
...
@pytest.mark.parametrize(
	"username,password", [("Mithrandir", "12345678"), ("Mithrandir"[::-1], "12345678")]
)
async def test_revoke_tokens_user_password_or_username_is_invalid_error(
	...
	response = await request_client.patch(
		request_url,
		headers={
			"Authorization": f"Basic {b64_encode_credentials(username, password)}"
		},
		data={"tokens": [uuid.uuid4(), uuid.uuid4()]},
	)
