...
async def app(token_validator_dependency_override) -> FastAPI:
    ...
    app.dependency_overrides = {jwt_validator_provider: token_validator_dependency_override}
    return app
...
async def users_raw_passwords(get_random_password):
    return {
        name: get_random_password(length=auth_settings.MIN_PASSWORD_LENGTH) for name in [
            'Gevorji', 'Mithrandir', 'Bilbo_baggins', 'The_Goblin_King'
        ]
    }
...
async def existing_tokens(user, token_issuer, db_session) -> dict[str, EncodedToken]:
    token_iss_args = {
        ...
        'scope': ['test1', 'test2'], 'private_claims': {'device_id': 'none'}
    }
    ...
    deviced_token_args = {
        'subject': user.username, 'scope': ['test1', 'test2'], 'private_claims': {'device_id': 'tel'}
    }
    deviced_access_token = EncodedToken(*token_issuer.get_access_token(**deviced_token_args))
    deviced_refresh_token = EncodedToken(*token_issuer.get_refresh_token(**deviced_token_args))
    for token in [access_token, refresh_token, deviced_access_token, deviced_refresh_token]:
        type_= 'access' if token.payload['scope'][0] != 'refresh' else 'refresh'
        await add_token_state_to_db(
            TokenState(
                id=token.payload['jti'], user_id=user.id, type=type_,
                device_id=token.payload['device_id'], expiry_date=datetime.datetime.fromtimestamp(token.payload['exp']),
            ), db_session
        )

    tokens = {
        token_iss_args['private_claims']['device_id']:{'access': access_token, 'refresh': refresh_token},
        deviced_token_args['private_claims']['device_id']:{'access': deviced_access_token, 'refresh': deviced_refresh_token}
    }
...
async def existing_revoked_refresh_token(user, token_issuer, db_session):
    ...
    await add_token_state_to_db(
        TokenState(
            id=token.payload['jti'], user_id=user.id, type='refresh', revoked=True,
