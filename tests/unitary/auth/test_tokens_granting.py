...
async def test_token_granting_successful(
    ...
    existing_refresh_token = TokenState(
        id=uuid.uuid4(), user_id=user.id, type='refresh',
        device_id='none', expiry_date=datetime.datetime.now()+datetime.timedelta(days=10)
    )
    existing_access_token = TokenState(
        id=uuid.uuid4(), user_id=user.id, type='access',
        device_id='none', expiry_date=datetime.datetime.now()+datetime.timedelta(minutes=15)
    )
    ...
    response = await request_client.post(
        request_url, data={'username': user.username, 'password': users_raw_passwords[user.username]}
    )
...
async def test_token_granting_invalid_password_failure(request_url, user, request_client, users_raw_passwords):
    response = await request_client.post(
        request_url, data={'username': user.username, 'password': users_raw_passwords[user.username][::-1]}
    )
    assert response.status_code == 401


async def test_token_granting_nonexistent_user_failure(request_url, user, request_client, users_raw_passwords):
    response = await request_client.post(
        request_url, data={'username': user.username[::-1], 'password': users_raw_passwords[user.username]}
    )
    assert response.status_code == 401


async def test_token_granting_inactive_user_failure(request_url, users_models, request_client, users_raw_passwords):
