...
async def jwt_validator_revocation_stub(encryption_key):
    return partial(
        validate_jwt, token_validator=JWTValidator(key=encryption_key.as_pem(private=False)),
...
async def get_mock_token(encryption_key):
    def _get_mock_token(**kwargs) -> EncodedToken:
        ...
        return EncodedToken(str=jwt.encode(header, payload, encryption_key), payload=payload, header=header)
...
def randomize_char(_string: str) -> str:
    ...
    _string = _string[:idx] + random.choice(string.ascii_letters+string.digits) + _string[idx+1:]
    return _string
...
@pytest.mark.parametrize(
    'iat,nbf,exp',
    [
        (now:=datetime.datetime.now(), now+datetime.timedelta(minutes=1), now+datetime.timedelta(minutes=5)),
...
@pytest.mark.parametrize(
    ...
    [
        (
                base:=datetime.datetime.now(tz=datetime.timezone.utc)-datetime.timedelta(minutes=5),
                base-datetime.timedelta(minutes=4),
                datetime.datetime.now()-datetime.timedelta(seconds=10),
        ),
        (
                base, base+datetime.timedelta(minutes=1), datetime.datetime.now()-datetime.timedelta(seconds=10),
        ),
        (
...
async def test_validate_jwt_expired_token_failure(iat, nbf, exp, get_mock_token, jwt_validator):
    token = get_mock_token(iat=iat, nbf=nbf, exp=exp)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
                base, base+datetime.timedelta(minutes=6), base+datetime.timedelta(minutes=2),
        ),
        (
...
async def test_validate_jwt_inconsistent_iat_exp_failure(iat, nbf, exp, get_mock_token, jwt_validator):
    token = get_mock_token(iat=iat, nbf=nbf, exp=exp)
...
async def test_validate_jwt_invalid_signature_failure(get_mock_token, jwt_validator):
    ...
    token = get_mock_token(iat=now, nbf=now+datetime.timedelta(minutes=1), exp=now+datetime.timedelta(minutes=5))
    token_header_str, token_payload_str, token_signature_str = token.str.split('.')
    corrupted_tokens = [
        token_header_str + '.' + randomize_char(token_payload_str) + '.' + token_signature_str,
        token_header_str + '.' + token_payload_str + '.' + randomize_char(token_signature_str),
        ]
