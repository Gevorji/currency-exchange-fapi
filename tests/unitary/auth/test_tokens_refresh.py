...
async def test_tokens_refresh_successful(
	...
	response = await request_client.post(
		...
		data={
			...
			"refresh_token": existing_tokens["none"]["refresh"].str,
		},
	)
	...
	revoked_refresh_token_state = await get_token_state_from_db(
		existing_tokens["refresh"].payload["jti"], db_session
	)
	revoked_access_token_state = await get_token_state_from_db(
		existing_tokens["access"].payload["jti"], db_session
	)
...
async def test_tokens_refresh_invalid_password_error(
	...
	response = await request_client.post(
		request_url,
		headers={
			"Authorization": f"Basic {b64_encode_credentials(user.username, users_raw_passwords[user.username][::-1])}"
		},
		data={
			"grant_type": "refresh_token",
			"refresh_token": existing_tokens["none"]["refresh"].str,
...
async def test_tokens_refresh_revoked_token_error(
	...
	response = await request_client.post(
		...
		data={
			"grant_type": "refresh_token",
			"refresh_token": existing_revoked_refresh_token.str,
		},
	)
