...
class CurrencyCode(UserString):
	def __init__(self, code: str, *args: Any, **kwargs: Any) -> None:
		if (
			not code.isalpha()
			or len(code) != 3
			or not set(code).issubset(ascii_letters)
		):
			raise errors.IncorrectCurrencyCodeError(
				"Currency code must consist of 3 alphabetic characters"
			)
...
class CurrencyName(UserString):
	def __init__(self, name: str, *args: Any, **kwargs: Any) -> None:
		...
		if not set(name).issubset(ascii_letters + " "):
			raise errors.IncorrectCurrencyName(
				"Currency name must consist of ascii letters and optionally spaces"
			)
...
class ExchangeRateValue:
	def __init__(self, value: int | float | Decimal):
		if not value > 0 or value in [float("inf"), float("-inf")]:
			raise errors.IncorrectExchangeRateValue(
				f"Invalid value for exchange rate {value}"
			)
		self.value = value
