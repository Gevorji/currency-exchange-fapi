...
currency_code_not_present_exc = HTTPException(
...
@currencies_router.get(
	...
	response_model=CurrencyOutSchema,
	responses={
...
async def get_currency(currency_code: CurrencyCodeField, user: user_dependency):
	try:
		...
		logger.debug(
			"Error requesting currency %s by user %s",
			currency_code,
			user.username,
			exc_info=True,
		)
...
async def add_currency(
	...
	try:
		...
		logger.debug(
			"Error on trying to add currency by user %s", user.username, exc_info=True
		)
		raise HTTPException(
			status_code=status.HTTP_409_CONFLICT,
			detail=f"Currency with code {new_currency.code} already exists",
		)
...
async def update_currency(
	currency_code: currency_code_path_field,
	currency_data: Annotated[UpdateCurrencySchema, Form()],
	user: user_dependency,
):
	try:
		upd_currency = await currency_exchange_app.update_currency(
			currency_code, currency_data
		)
		logger.info(
			"User %s updated currency %s, with %s",
			user.username,
		...
		logger.debug(
			"Error on trying to update currency by user %s",
			user.username,
			exc_info=True,
		)
		raise HTTPException(
...
async def delete_currency(
	currency_code: currency_code_path_field, user: user_dependency
):
	try:
		deleted_currency = await currency_exchange_app.delete_currency(currency_code)
		logger.info(
			"User %s deleted currency %s, %s",
			deleted_currency.code,
			deleted_currency.name,
		)
	except appexc.CurrencyDoesNotExistError:
		logger.debug(
			"Error on trying to delete currency by user %s",
			user.username,
			exc_info=True,
		)
		raise HTTPException(
