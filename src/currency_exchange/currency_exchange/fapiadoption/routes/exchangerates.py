...
er_codes_not_present_exc = HTTPException(
	status_code=status.HTTP_400_BAD_REQUEST,
	detail="Request path to this resource should be of a form /currency/{code}",
)
...
async def get_exchange_rate(
	...
	try:
		...
		logger.debug(
			"Error on trying to get currency by user %s", user.username, exc_info=True
		)
...
@exchange_rates_router.post(
	...
	response_model=ExchangeRateOutSchema,
	responses={
		409: {"description": "Exchange rate already exists"},
...
async def add_exchange_rate(
	new_exchange_rate: Annotated[AddExchangeRateSchema, Form()], user: user_dependency
):
	try:
		try:
			logger.debug(
				"User %s adding exchange_rate %s", user.username, new_exchange_rate
			)
			added_er = await currency_exchange_app.add_exchange_rate(new_exchange_rate)
			logger.info(
				"User %s added exchange rate %s", user.username, new_exchange_rate
			)
			return added_er
		except Exception:
			logger.debug(
				"Error on trying to add exchange_rate %s by user %s",
				new_exchange_rate,
				user.username,
				exc_info=True,
			)
		...
		raise HTTPException(
			status_code=409, detail="Can't add exchange rate to same currency"
		)


@exchange_rates_router.patch(
	"/exchangerate/{code_pair}",
	response_model=ExchangeRateOutSchema,
	responses={
		404: {"description": "Currency(ies) not found"},
...
async def update_exchange_rate(
	code_pair: Annotated[str, CodePairPathField],
	er_data: Annotated[UpdateExchangeRateSchema, Form()],
	user: user_dependency,
):
	base_code, target_code = code_pair[:3], code_pair[3:]
	try:
		try:
			upd_er = await currency_exchange_app.update_exchange_rate(
				base_code, target_code, er_data.rate
			)
			logger.debug(
				"User %s updating exchange_rate %s-%s with %s",
				user.username,
				base_code,
				target_code,
				er_data.model_dump(exclude_unset=True),
			)
			logger.info(
				"User %s updated exchange rate %s-%s with %s",
				user.username,
				base_code,
				target_code,
				er_data.model_dump(exclude_unset=True),
			...
			logger.debug(
				"Error on trying to update exchange_rate %s by user %s",
...
async def delete_exchange_rate(
	...
	base_code, target_code = code_pair[:3], code_pair[3:]
	try:
		deleted_er = await currency_exchange_app.delete_exchange_rate(
			base_code, target_code
		)
		logger.info(
			"User %s deleted exchange rate %s-%s", user.username, base_code, target_code
		)
		return deleted_er
	except appexc.CurrencyDoesNotExistError as e:
		logger.debug(
			"Error on trying to delete exchange_rate %s-%s by user %s",
			base_code,
			target_code,
			user.username,
			exc_info=True,
