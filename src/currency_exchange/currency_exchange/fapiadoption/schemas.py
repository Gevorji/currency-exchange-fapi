...
user_string_converter = BeforeValidator(
	lambda v: v.data if isinstance(v, UserString) else v
)
exchange_rate_value_converter = BeforeValidator(
	lambda v: v.value if isinstance(v, ExchangeRateValue) else v
)
CurrencyCodeField = Annotated[
	str, Field(pattern=re.compile("^[A-Z]{3}$")), user_string_converter
]
...
class UpdateCurrencySchema(BaseModel):
	new_name: Annotated[
		CurrencyNameField, Field(max_length=40, validation_alias="name")
	]
...
class ExchangeRateOutSchema(BaseModel):
	...
	targetCurrency: Annotated[
		CurrencyOutSchema, Field(validation_alias="target_currency")
	]
...
class UpdateExchangeRateSchema(BaseModel):
	rate: Annotated[
		float, Field(gt=0, allow_inf_nan=False), AfterValidator(lambda v: round(v, 6))
	]
