...
class CurrenciesExchangeRateORMModel(BaseModel):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    base_crncy_id: Mapped[int] = mapped_column(ForeignKey('currency.id', ondelete='CASCADE'))
    target_crncy_id: Mapped[int] = mapped_column(ForeignKey('currency.id', ondelete='CASCADE'))
