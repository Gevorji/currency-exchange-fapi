...
from ...application.extdm import (IdentifiedCurrency as Currency,
                                  IdentifiedCurrenciesExchangeRate as CurrenciesExchangeRate)
from .dbmodels import CurrencyORMModel as CurrencyORM, CurrenciesExchangeRateORMModel as ExRateORM, \
    CurrenciesExchangeRateORMModel
from ...application.interfaces import CurrencyRepoInterface, ExchangeRatesRepoInterface
from ...application.dto import (GetCurrencyDto, GetExchangeRateDto, AddCurrencyDto, AddExchangeRateDto,
...
class CurrencyPostgresRepo(CurrencyRepoInterface):
            ...
            async with session.begin():
                try:
                    res = await session.scalars(
                        ...
                        [{'code': currency.code.data, 'name': currency.name.data, 'sign': currency.sign.data}]
                    )
                except sqlalchemy.exc.IntegrityError as e:
                    raise errors.CurrencyAlreadyExistsError(f'Currency with code {currency.code} already exists') from e

        return orm_currency_to_dm_currency(res.one())

    async def update_currency(self, currency: AlterCurrencyDto) -> Currency:
        update_values = {k.removeprefix('new_'): v.data for k, v in dataclass_asdict(currency).items()
                         if v is not None and k != 'code'}
        async with self._session_factory() as session:
            async with session.begin():
                res = await session.scalars(
                    update(CurrencyORM).where(CurrencyORM.code == currency.code.data).values(**update_values)
                    .returning(CurrencyORM),
                )
        try:
            return orm_currency_to_dm_currency(res.one())
        except NoResultFound:
            raise errors.CurrencyDoesNotExistError(f'No currency with code {currency.code} to alter')
    ...
    async def delete_currency(self, currency: DeleteCurrencyDto) -> Currency:
        async with self._session_factory() as session:
            async with session.begin():
                res = await session.scalars(
                    delete(CurrencyORM).where(CurrencyORM.code == currency.code.data).returning(CurrencyORM)
                )
        try:
            return orm_currency_to_dm_currency(res.one())
        except NoResultFound:
            raise errors.CurrencyDoesNotExistError(f'No currency with code {currency.code} to delete')
...
class ExchangeRatesPostgresRepo(ExchangeRatesRepoInterface):
    ...
    async def get_rate(self, rate: GetExchangeRateDto) -> CurrenciesExchangeRate:
        async with self._session_factory() as session:
            ...
            res = await session.scalars(
                select(ExRateORM)
                .join(base, ExRateORM.base_crncy).where(base.code == rate.base_currency.data)
                .join(target, ExRateORM.target_crncy).where(target.code == rate.target_currency.data)
            )
        try:
            return orm_ex_rate_to_dm_ex_rate(res.one())
        except NoResultFound:
            raise errors.ExchangeRateDoesntExistError(f'No exchange rate '
                                                      f'for {rate.base_currency}->{rate.target_currency}')
        ...
        async with self._session_factory() as session:
            async with session.begin():
                try:
                    ...
                    raise ExchangeRateAlreadyExistsError(f'Exchange rate {rate.base_currency}-{rate.target_currency} '
    ...
    async def get_cross_rates(
        ...
        base_crncy_id = id_res[rate.base_currency]
        target_crncy_id = id_res[rate.target_currency]
        base_crncy_rates_cte = select(ExRateORM).where(
            or_(
                and_(ExRateORM.base_crncy_id == base_crncy_id, ExRateORM.target_crncy_id != target_crncy_id),
                and_(ExRateORM.base_crncy_id != target_crncy_id, ExRateORM.target_crncy_id == base_crncy_id)
            )
        ).cte()
        target_crncy_rates_cte = select(ExRateORM).where(
            or_(
                and_(ExRateORM.base_crncy_id == target_crncy_id, ExRateORM.target_crncy_id != base_crncy_id),
                and_(ExRateORM.base_crncy_id != base_crncy_id, ExRateORM.target_crncy_id == target_crncy_id)
            )
        ...
        async with self._session_factory() as session:
            res = await session.execute(
                select(base_crncy_rates, target_crncy_rates).join_from(
                    base_crncy_rates, target_crncy_rates, or_(
                        base_crncy_rates.base_crncy_id == target_crncy_rates.base_crncy_id,
                        base_crncy_rates.base_crncy_id == target_crncy_rates.target_crncy_id,
                        base_crncy_rates.target_crncy_id == target_crncy_rates.base_crncy_id,
                        base_crncy_rates.target_crncy_id == target_crncy_rates.target_crncy_id,
                    ),
                )
            )
    ...
    async def _get_currencies_ids(self, *currencies: *Sequence[CurrencyCode]) -> dict[CurrencyCode, int]:
        async with self._session_factory() as session:
            id_res = await session.execute(
                select(CurrencyORM.id, CurrencyORM.code).where(CurrencyORM.code.in_(currencies))
            )
        id_res = {code: id_ for id_, code in id_res}
        notfound = set(code for code in id_res).symmetric_difference(set(currencies))
        if notfound:
            raise errors.CurrencyDoesNotExistError(f'Currency(ies) with code(s) {", ".join(notfound)} not found')
