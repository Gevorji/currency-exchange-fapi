...
admin_router = APIRouter(
	prefix="/admin", dependencies=[Security(verify_access, scopes=["all"])]
)
...
additional_openapi_responses_for_users = {404: {"description": "User not found"}}
...
async def change_user_is_active(
	user: UserDbOut, is_active: bool, conflict_msg: str
) -> UserDbOut:
...
async def change_users_category(
	...
	try:
		...
		raise user_not_found_exception
	if direction == "up":
		not_consistent_with_current_state = (
			to < TransitionUsersCategories[promoted_user.category.name]
		)
		msg_409 = "lower"
	else:
		not_consistent_with_current_state = (
	...
	if to == TransitionUsersCategories[promoted_user.category.name]:
		raise HTTPException(
			status.HTTP_409_CONFLICT, detail=f"User already has category {to.name}"
		)
...
@users_ops_router.get(
	"/all", status_code=status.HTTP_200_OK, response_model=list[UserOut]
)
...
async def make_user_inactive(user_id: int):
	...
	return await change_user_is_active(
		user, is_active=False, conflict_msg="User is admin or deactivated already"
	)
...
async def make_user_active(user_id: int):
	try:
		...
		raise user_not_found_exception
	return await change_user_is_active(
		user, is_active=True, conflict_msg="User is admin or is active already"
	)


@users_ops_router.patch(
	"/{user_id}/category/promote",
	status_code=status.HTTP_200_OK,
	response_model=UserOut,
	responses={
		409: {
			"description": "Performed update conflicts with current state of user category"
		},
		**additional_openapi_responses_for_users,
	},
)
