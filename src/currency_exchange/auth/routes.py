...
@clients_router.post(
	...
	responses={
		400: {
			...
			"description": "Submitted values invalid",
		},
...
async def create_user(
	...
	if password1 != password2:
		...
		return JSONResponse(
			jsonable_encoder(
				UserCreationErrorResponse.model_validate(
	...
	try:
		try:
			...
			logger.info("Unsuccessful attempt to register user %s", username)
			logger.debug(
				"Exception info of unsuccessful registration", username, exc_info=e
			)
			raise
...
@token_router.post(
	...
	responses={
		...
		401: {"description": "Invalid credentials"},
		403: {
			"description": "User disabled or token owner is not a user, or token is revoked"
		},
	},
)
async def refresh_access_token(
	...
	grant_type: Annotated[
		...
		Form(description='Required to be an exact "refresh_token" value'),
	],
	...
	try:
		try:
			...
			if token_owner_id != user.id:
				raise HTTPException(
					detail="Token owner does not match with client from request",
	...
	previous_access_scopes = token.claims.scope[
		1:
	]  # first scope will always be 'refresh'

	token_issuer = JWTIssuerProvider(user, token.claims.device_id)
	access_token_str, _, access_token_payload = token_issuer.get_access_token(
		scope=previous_access_scopes
	)
	refresh_token_str, _, refresh_token_payload = token_issuer.get_refresh_token(
...
@token_router.patch(
	...
	response_model=TokensRevokedResponse,
	responses={
		401: {"description": "Invalid credentials"},
		403: {"description": "User disabled"},
	},
